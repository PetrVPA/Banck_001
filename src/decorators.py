...
def log(filename=None):
    '''
    Функция выполняет логирование обертываемой функции в файл если указано имя файла или консоль если имя
    не передается.В лог записывается то что операция выполнена успешно ("my_function ok\n"), либо сообщает
    тип ошибки и введенные переменные при неудачном выполнении обертываемой функции, например
